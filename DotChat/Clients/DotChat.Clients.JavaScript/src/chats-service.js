...
export class ChatsService{
    ...
    */
    ...
    async editMessage(chatId, messageId, messageInfo){
        ...
        var readers = this._getChatMessageReaders(chatId);
        readers.forEach(reader => reader.addOrUpdate(messageId, message, exist => _.merge(exist, messageInfo, {
            pending: true,
    ...
    _getChatReaders(){
        var readers = this._chatsReaders.named.entries().map(([_, value]) => value).filter(r => !r.filter).map(r => r.reader);
    ...
    _registerChatsNotifications(){
        this._connector.chats.chatAdded = (notification) => {
            ...
            for(namedChatReaderBox in this._chatsReaders.named.entries().map(([_,value]) => value)){
    ...
    _getChatMessageReaders = (chatId) => {
        var chatMessagesReaders = this._messagesReaders[chatId];
        if(chatMessagesReaders){
            var readers = chatMessagesReaders.named.entries().map(([_, value]) => value).filter(r => !r.filter).map(r => r.reader);
            readers.push(this._chatsReaders.default);
            return readers;
        }else{
            return [];
